...
Для email (скидання пароля, надсилання доступу) налаштуйте SMTP у адмінці: **Панель → Користувачі → Пошта (SMTP)**, або `SMTP_*` у `.env`. Без SMTP пароль показується адміністратору, щоб передати учню вручну.
